...
'use strict';
//import {Wallets,Gateway,X509WalletMixin} from 'fabric-network';
...
exports.connectToNetwork = async function (userName) {
  ...
  try {
    ...
    const contract = await network.getContract('EvoteContract');
...
exports.registerVoter = async function (voterId, gender, age,password,checkPassword) {


  console.log('voterId ');
  console.log(voterId);

  if (!voterId || gender==null || age==null||!password||!checkPassword) {
    let response = {};
  ...
  if (password!=checkPassword) {
    let response = {};
    response.error = 'password dont match';
    return response;
  }
  ...
  try {
   ...
   // await gateway.connect(ccp, { wallet, identity: appAdmin, discovery: gatewayDiscovery });
    ...
    const secret = await ca.register({ affiliation: '', enrollmentID: voterId, role: 'client' }, adminUser);
    ...
    let response = `Successfully registered. Use voterId ${voterId} to login above.`;
    return response;
  } catch (error) {
